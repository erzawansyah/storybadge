...
const InputAddressForm: FC<InputAddressFormProps> = ({ onSubmit, onChange, isLoading, buttonColor }) => {
    const [address, setAddress] = useState<string>('');
    const [error, setError] = useState<string | null>(null);
    buttonColor = buttonColor || 'gradient-2';
    const buttonColorReverse = buttonColor + '-reverse';
    ...
    const handleOnChange = (e: FormEvent<HTMLFormElement>) => {
        ...
        if (onChange) onChange(e.target as unknown as FormEvent<HTMLInputElement>);
    };
    ...
    return (
        ...
        >
            ...
            <button
                type="submit"
                className={`w-full p-3 ${buttonColor} text-white font-semibold rounded-lg shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition ${isLoading
                    ? 'opacity-70 cursor-not-allowed'
                    : `hover:${buttonColorReverse} hover:shadow-blue-500/50`
                    }`}
                disabled={isLoading}
            >
                {isLoading ? (
